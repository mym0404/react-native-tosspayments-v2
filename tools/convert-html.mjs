...
await iterateDir('src/html', (file) => {
  const [name, ext] = file.split('.');
  if (ext !== 'html') return;
  const p = join('src/html', file);
  const html = read(p);
  const ret = `export const ${name} = \`${html}\`;\n`.trim();
  const outputPath = join(`src/html/${name}.ts`);
  write(outputPath, ret);
  printSuccess(`Write ${outputPath}`);
});
